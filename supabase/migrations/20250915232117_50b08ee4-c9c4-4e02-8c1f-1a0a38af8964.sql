...
-- This is essential for proper RLS policies
...
-- No DELETE policy means no one can delete orders except admins through direct SQL

-- For admin access, we'll create a separate policy later when admin roles are implemented
-- For now, direct database access by authorized personnel only

-- Add index for better performance on user_id queries
CREATE INDEX IF NOT EXISTS idx_orders_user_id ON public.Orders(user_id);

-- Update the updated_at trigger for Orders table if it doesn't exist
